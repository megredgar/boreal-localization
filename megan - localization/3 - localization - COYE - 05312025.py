# -*- coding: utf-8 -*-
"""
Localization of [[Common Yellowthroat (COYE)]] calls using opensoundscape
...
# =============================================================================
aru_coords_path   = r"D:/BARLT Localization Project/localization_05312025/hawkears_0_4thresh_COYE/aru_coords.csv"
detections_path   = r"D:/BARLT Localization Project/localization_05312025/hawkears_0_4thresh_COYE/detections_COYE.csv"
shelf_out_path    = r"D:/BARLT Localization Project/localization_05312025/hawkears_0_4thresh_COYE/pythonoutput/coye.out"
...
# =============================================================================
# Load detections
# detections_coye.csv should have: file, start_time, end_time, coye
# =============================================================================
...
print(f"The receivers on which coye was detected: \n{example.receiver_files}")
...
plt.xlabel("Easting")
plt.ylabel("Northing")
plt.title("Example COYE localization")
plt.show()
...
plt.title("Aligned spectrograms for example COYE event")
plt.show()

print(f"Residual RMS for example event: {example.residual_rms:.2f} m")

# =============================================================================
# All COYE estimates for the same time window as the example
...
x_coords = [e.location_estimate[0] for e in coye_same_event]
y_coords = [e.location_estimate[1] for e in coye_same_event]
rms = [e.residual_rms for e in coye_same_event]
...
plt.ylabel("Latitude")
plt.title("COYE localization (single time window)")
plt.show()
...
# =============================================================================
# Residual RMS summary across all coye estimates
# =============================================================================
coye_all = [e for e in position_estimates if e.class_name == "COYE"]

residuals = [e.residual_rms for e in coye_all]
...
print("Residual RMS (COYE events):")
...
rms_cutoff = 35  # m
low_rms = [e for e in coye_all if e.residual_rms < rms_cutoff]
...
plt.scatter(
    ...
    edgecolor="black",
    alpha=0.6,
    label=f"COYE (RMS < {rms_cutoff} m)",
)
plt.legend(bbox_to_anchor=(1.02, 1), loc="upper left")
plt.xlabel("Easting")
plt.ylabel("Northing")
plt.title("COYE localizations with low residual error")
plt.show()

# =============================================================================
# Average COYE  position per time window (for good localizations)
# =============================================================================
rms_threshold = 20  # meters, residual RMS threshold for “high-confidence” events

coye_events = [
    e for e in position_estimates
    if e.class_name == "COYE" and e.residual_rms < rms_threshold
]

grouped_by_time = defaultdict(list)
for event in coye_events:
...
if len(avg_locations) == 0:
    ...
    plt.scatter(
        x_avg_coords,
        y_avg_coords,
        label="Average COYE location per time window",
        alpha=0.7,
    )
    plt.scatter(
        x_known,
    ...
    plt.xlabel("Longitude")
    plt.ylabel("Latitude")
    plt.title("Average COYE localization per time window")
    plt.legend(bbox_to_anchor=(1.02, 1), loc="upper left")
    plt.grid(False)
    plt.show()
